'''
# Purpose
This notebook is aligned to deliverable 1.  Specifically:
...
class DataProcessor:
    ...
    def preprocess(self) -> None:
        ...
        self.df['arrival_date_full'] = pd.to_datetime(
            dict(year=self.df['arrival_year']
    ...
    def enable_change_data_feed(self) -> None:
        ...
        self.spark.sql(
            f"ALTER TABLE {catalog_path}.train_set "
            "SET TBLPROPERTIES (delta.enableChangeDataFeed = true);"
        )

        self.spark.sql(
            f"ALTER TABLE {catalog_path}.test_set "
            "SET TBLPROPERTIES (delta.enableChangeDataFeed = true);"
        )
